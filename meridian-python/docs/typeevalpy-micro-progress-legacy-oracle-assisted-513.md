# Meridian × TypeEvalPy micro progress (513 inventory) — LEGACY ARCHIVE

> **Historical only — not a production SOTA claim.**
> This run used **oracle-assisted pairing**: when multiple sites shared a
> locator, the scorer could pick the candidate whose predicted types matched
> `expected_types`. Keep for reproducibility of the pre-generalization
> `513/513` headline. Primary gates are now `strict` / `compat` in
> `typeevalpy-micro-progress.md` (compat never uses expected types).
